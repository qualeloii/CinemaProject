...
public class GuardiansGalaxyControl extends Control{
	...
	public void InsereDados(Assento assento) {
		...
		try {
			...
			String sSQL = "Insert INTO cinema.guardians_galaxy Values(null,'"+assento.GetAssento()+"', '"+this.preco+"');";
	...
	public boolean HasComprado(String cadeira) {
		...
		try {
			Connection ExConn = BD.abrirBD();
			Statement Stmt = (Statement) ExConn.createStatement();
			ResultSet rs = Stmt.executeQuery("Select id, cadeira, valor FROM guardians_galaxy");
			while(rs.next()) {
